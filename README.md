...
- The corridor-aware tracker is experimental and is deliberately not the default. Sparse-corridor parents are hypotheses for review, not declarations that several traces are one OSM way. Promotion still requires the full fixture suite plus real exported/rated examples across all supported palettes.
...
- `Ridge tracker`: leave `Legacy v0.2-compatible` selected for established behavior. Select `Experimental corridor-aware` to enable broad-corridor centering, sparse longitudinal inference, parallel parent/child candidates, and pre-optimized endpoint approaches. A preview label such as `Hot detector - sparse corridor 1` is the additional parent hypothesis; ordinary `strand` candidates are its retained elementary alternatives. This is an independent choice from `Alignment mode`.
