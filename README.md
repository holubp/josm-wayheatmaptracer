...
- `Show aggregate intensity layer`: when managed all-color aggregation is enabled, adds a non-editable `WayHeatmapTracer aggregate intensity` layer after a slide. The layer visualizes the exact scalar aggregate field used by `all-colors-combined`: weak signal is blue/cyan, stronger signal is red/magenta, and saturated maxima become pale/white. It is diagnostic only and does not change alignment.
