...
- `Show aggregate intensity layer`: adds a non-editable `WayHeatmapTracer aggregate intensity` layer after a managed-source slide, even if the aggregate detector candidate itself is disabled. The layer visualizes the exact scalar aggregate field used by `all-colors-combined`: weak signal is blue/cyan, stronger signal is red/magenta, and saturated maxima become pale/white. It is diagnostic only and does not change alignment.
