...
- Choose between the default `Legacy v0.2-compatible` ridge tracker and an opt-in `Experimental corridor-aware` tracker that uses nested corridor boundaries, continuous signal/localization confidence, longitudinal strand identity, and second-order centerline optimization
- In corridor-aware mode, retain separate child strands and parent-corridor hypotheses when lane-like traces share a shoulder; keep deep-valley parallel carriageways separate and show ambiguous cases as alternatives
- In corridor-aware mode, map RGB to scalar intensity first, then build local L0/L1/L2 Gaussian levels from the imagery already acquired for the slide. Coarse evidence stabilizes broad plateaus but never averages completed candidate lines or moves a fine child to a merged parallel parent.
- In corridor-aware mode, constrain fixed anchors before geometry is built and keep movable endpoints/junctions within both a source-position prior and a hard 10 metre/configured-search limit
...
- Keep no-signal, too-weak, and structurally unsafe detector outcomes visible for diagnosis; inspection-only candidates are dashed red and cannot be applied, while short unsupported gaps may still be bridged between real signal
...
- The corridor-aware tracker is experimental and is deliberately not the default. Its promotion requires the full fixture suite plus real exported/rated examples across all supported palettes.
...
- `Ridge tracker`: leave `Legacy v0.2-compatible` selected for established behavior. Select `Experimental corridor-aware` when testing stable broad-corridor centering, sparse-strand persistence, parallel parent/child candidates, or pre-optimized endpoint approaches. This is an independent choice from `Alignment mode`.
...
- `candidate-metrics.csv`, with detector, visible color, intensity source, source tier, applicability, raw score, calibrated score, support ratio, mean intensity, mean gradient strength, longitudinal and scale stability, SNR, ambiguity, corridor existence/localization confidence, optimizer cost, in-corridor fraction, roughness, source-pixel-scale roughness, edge-pinning, and safety warnings for each candidate
...
- `optimizer-costs.csv`, with the selected offset and data, continuity, acceleration, endpoint, containment, and total costs for every profile/track
