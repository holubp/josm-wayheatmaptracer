...
- An enabled choice may add one cleaned sibling while retaining the raw candidate. The preview opens on a valid changed cleaned sibling of the highest-ranked applicable base candidate and reports whether cleanup was unchanged, partial, fully applied, skipped, or rejected. Unsupported, interpolated, off-raster, no-signal, and scale-conflicted points remain exact local boundaries; they do not automatically disable cleanup on independent valid intervals.
