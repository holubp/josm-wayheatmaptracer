...
- `candidate-metrics.csv`, with detector, visible color, intensity source, raw score, calibrated score, support ratio, mean intensity, mean gradient strength, longitudinal stability, SNR, ambiguity, corridor existence/localization confidence, optimizer cost, in-corridor fraction, roughness, source-pixel-scale roughness, edge-pinning, and safety warnings for each candidate
