...
- In corridor-aware mode, constrain fixed anchors before geometry is built, derive endpoint approach direction from the selected branch rather than the connected road, and keep movable endpoints/junctions within both a source-position prior and a hard 10 metre/configured-search limit. An endpoint without reliable branch evidence remains visible for diagnosis but cannot be applied.
...
- The corridor-aware tracker is experimental and is deliberately not the default. It now uses a robust longitudinal tube and exact pair-state optimizer, but its promotion still requires the full fixture suite plus real exported/rated examples across all supported palettes.
