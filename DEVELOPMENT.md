...
7. When multi-color detection is enabled, each palette classifier is run independently against the same rendered layer. The preview can show those alternatives, but 0.8.x does not synthesize fused consensus geometry. Experimental `*-corridor`, `*-cool`, and `*-strict` detectors are preview/rating variants only; keep baseline detector behavior intact unless exported calibration data proves a variant should replace it.
