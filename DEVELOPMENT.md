...
7. Gaussian levels sample the same anchors and normals and express every band back in common L0 sampled-raster coordinates. Compatible levels require at least 50% shoulder overlap relative to the narrower band or center separation within `max(0.5 source pixel, combined uncertainty)`. Persistence weights are L0 `0.50`, L1 `0.35`, and L2 `0.15`. Coarse evidence is an uncertainty-gated prior for one fine optimizer, never post-hoc geometry consensus. A coarse parent shared by fine parallel children contributes persistence but no midpoint; an incompatible nearby coarse trace is a conflict and contributes no prior.
...
9. `CorridorTracker` predicts lateral offset from the preceding physical slope. A residual above 1.5 source pixels requires coherent same-direction support in following profiles; `scaleConflict` and `parentMerge` evidence cannot authorize that move. Unsupported gaps are bounded by both 16 profiles and 20 metres.
10. `CorridorTubeBuilder` fits a local line to the selected strand against cumulative metres. Its approximately 10 metre support window is clamped to 5-9 observations. Two Huber reweighting iterations use `delta=max(0.5 source pixel, 1.5 weighted MAD)`; broad or weakly localized corridors increase uncertainty, while persistent weak narrow strands retain non-zero longitudinal support. The tube is a reference and uncertainty envelope, not a replacement geometry.
11. The optimizer's slope is `deltaOffsetPx / profileSpacingPx`; do not divide that dimensionless value by source-pixel size again. Curvature uses actual candidate segment headings relative to the robust tube heading and mean source-pixel spacing. Within a broad intensity plateau, variations inside `epsilon=clamp(0.02 + 0.5*mean(|raw-B3|,|B3-B5|),0.02,0.10)` have equal peak-fit cost, after which robust nested/core and compatible coarse centers determine location. Confirmed turns require a seven-profile coherent-motion window and retain at least 65% of normal curvature regularization.
12. `CorridorCenterlineOptimizer` keeps at most 21 lateral states per profile, including mandatory raw/B3/B5, core, shoulder, tube, compatible coarse, and endpoint-guide states. It solves the complete second-order dynamic program keyed by `(previousOffset,currentOffset)` with deterministic backpointers. For `P` profiles and at most `S` states, time is `O(P*S^3)` and pair-state memory is `O(S^2)`; there is no beam pruning, post-optimization moving average, geometry consensus, or hidden rerun. Default data weights are core distance `0.55`, shoulder distance `4.0`, tube center `0.55`, and compatible coarse center `4.0`.
13. Endpoint approaches search 8-15 metres inward for a reliable non-parent strand anchor, falling back to the farthest nearer reliable profile. Scale conflicts, parent merges, and unresolved multimodal profiles are skipped. A cubic Hermite guide joins the constrained endpoint to the selected branch tube using the branch tangent. The heatmap center pull tapers near that boundary while shoulder containment remains active. Fixed endpoints stay exact; movable endpoints remain opt-in and clamped. Missing reliable approach evidence is exported and penalizes ranking, but is not by itself a structural failure; actual foldback, terminal-turn, and connected-way checks decide applicability.
...
15. Elementary parallel strands remain separate through longitudinal association. `CorridorGrouping` uses persistent common envelopes and normalized valleys: `>=0.65` supports a parent corridor, `<=0.40` supports separate corridors, and the interval between remains ambiguous. Decisions require at least five common profiles and 60% common support. Parent and child candidates remain visible; heatmap evidence does not silently declare lane semantics.
16. Candidates carry diagnostic evidence metadata and projected `EastNorth` geometry for preview/export. Corridor-aware candidates additionally carry unweighted physical metrics: tube residual, seven-profile high-frequency residual, first/second lateral differences in source pixels, turns and curvature changes in degrees, forward-progress violations, unsupported excursions, physical gap length, endpoint turn, and true longitudinal persistence. Ranking compares this calibrated quality only within the existing source tier; baseline legacy scores and ordering remain unchanged.
...
19. `AlignmentService` projects the chosen candidate back into map coordinates and prepares either:
...
21. `ReplaceWaySegmentCommand` applies the precise-shape result by reusing existing nodes where possible, creating additional nodes when needed, and deleting dropped untagged/unreferenced nodes. Reused node target coordinates must be replayed on every execute/redo.
...
Old bundles remain readable; physical format-4 columns are blank when the old export did not contain them. These commands analyze exported outcomes and do not replay a new tracker implementation against old imagery.
