...
6. `TrackerMode.CORRIDOR_AWARE` is experimental. It keeps complete native/B3/B5 scalar cross-sections, then runs `CorridorExtractor`, `CorridorTracker`, `CorridorGrouping`, and `CorridorCenterlineOptimizer`. Nested levels are currently `q={0.60,0.72,0.84,0.92}` above the robust local noise floor. The optimizer uses first- and second-difference costs with no free post-optimization moving average. Fixed and movable endpoint/junction constraints enter this optimizer before preview geometry is built.
7. Corridor confidence is continuous. Signal existence combines prominence, boundary gradients, and raw/B3/B5 agreement. Localization confidence combines edge balance, nested-center agreement, core definition, and uncertainty. A weak isolated observation is provisional; longitudinal persistence is required to create a useful track. Do not reintroduce the legacy absolute `0.14`, `0.20`, or `0.30` gates into the corridor-aware path.
8. Elementary parallel strands remain separate through longitudinal association. `CorridorGrouping` uses persistent common envelopes and normalized valleys: `>=0.65` supports a parent corridor, `<=0.40` supports separate corridors, and the interval between remains ambiguous. Decisions require at least five common profiles and 60% common support. Parent and child candidates remain visible; heatmap evidence does not silently declare lane semantics.
9. Candidates carry diagnostic evidence metadata and projected `EastNorth` geometry for preview/export. Baseline legacy detector scores must remain the 0.2-style scores; multi-color default ordering may apply a separate calibrated ranking score, which must be exported alongside the raw score.
10. The current-source detector option and the managed all-color aggregate option are independent. When current-source alternative mappings are enabled, detector variants are applied to the selected rendered/manual source or the selected managed color source. In this codebase, detector variant names mean scalar intensity mappings; after intensity conversion, the selected tracker mode runs on that scalar field. When managed all-color aggregation is enabled, source-tile sampling downloads and caches the base Strava colors and adds `all-colors-combined`: each color is converted through its native semantic intensity mapping, the scalar intensity fields are fused with the calibrated weighted power mean (`p=1.25`), and only then are corridors/ridges extracted. The aggregate candidate requires complete matching `hot`, `blue`, `bluered`, `purple`, and `gray` source mosaics; partial aggregates must fail clearly. Finished candidate geometries are never merged across detectors.
11. `ParallelWayContextResolver` and `CorridorAssignmentService` run only when both corridor-aware tracking and the existing opt-in context setting are enabled. They inspect downloaded nearby `highway=*` geometry and limited relevant tags, adjust candidate ranking, and never edit contextual primitives.
12. `AlignmentService` projects the chosen candidate back into map coordinates and prepares either:
   `Move Existing Nodes` preview geometry
   `Precise Shape` preview geometry
13. `AlignWayAction` refuses candidate switching or apply if the modeless preview source way, segment node identities, dataset membership, or source coordinates changed after the slide was computed.
14. `ReplaceWaySegmentCommand` applies the precise-shape result by reusing existing nodes where possible, creating additional nodes when needed, and deleting dropped untagged/unreferenced nodes. Reused node target coordinates must be replayed on every execute/redo.
