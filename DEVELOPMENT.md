...
success separately in verbose diagnostics without opening a success message; failed or
...
5. `TrackerMode.CORRIDOR_AWARE` is the default from `0.20.0`. It maps source pixels to scalar intensity before filtering, crops to the selected search envelope plus halo, and builds local Gaussian levels. Each next level applies separable B5 `[1,4,6,4,1]/16` in both axes and globally fixed-phase 2x decimation. Managed native mosaics use exact 1x/2x/4x source pitches. The oversampled rendered fallback retains fine rendered L0 for localization and selects the nearest globally phased pyramid reductions around 2x and 4x its reported/estimated source-pixel pitch. Missing kernel support is invalid rather than zero-filled, the estimated extra value/mask storage is capped at 128 MiB, and no synthetic level performs a tile request. Complete all-color detection aggregates native scalar L0 fields first and builds one pyramid from the aggregate. Current-source alternatives each build from their own scalar mapping.
6. `TrackerMode.LEGACY_V02` remains an explicit compatibility fallback. It routes profiles through `RidgeTracker` and preserves the 0.2.0-compatible dynamic-programming behavior with the existing longitudinal consistency filter and anchored intervals. Do not change its peak extraction, smoothing, ranking, or fallback behavior as part of corridor-aware calibration or default promotion.
...
21. `ReplaceWaySegmentCommand` applies the precise-shape result by first validating the candidate-owned protected-node assignments against the shared pure planner, before any mutation. It then assigns reusable mutable nodes monotonically to preview path-length fractions with a bounded sequence dynamic program. Fixed and soft-anchor slots are excluded, unmatched preview slots create nodes, and dropped nodes are removed only when untagged and unreferenced. Reused node target coordinates are immutable command state and must be replayed on every execute/redo. Replacement planning completes without dataset mutation before the base JOSM command snapshots every affected primitive; undo therefore restores the pre-slide primitive modified flags as well as topology and coordinates.
...
The `0.17.1` and `0.18.0` review identified compatibility and evidence-boundary defects without changing either tracker objective. Legacy precise candidates deliberately expose no existing-node assignment map, so precise replacement treats null and empty maps as the legacy planning path. Corridor-aware precise candidates require their immutable assignment map and fail closed when it is missing. Replacement planning records reused-node targets without mutating dataset coordinates; execute/redo remains the only mutation phase.

Corridor grouping now requires the full independent coverage, physical span, stable ordering, valley, and separation proof before declaring persistent parallel strands separate. A short deep-valley overlap remains an ambiguous parent/children interpretation. Predicted parent contributors may define the interpolated geometric envelope, but only direct-union contributors supply observed signal statistics or longitudinal motion support. Interpolation-only parent profiles keep the 5/12/32-metre windows physically bounded instead of importing distant observations to meet minimum sample counts.

Final-preview topology validation includes proper crossings, endpoint/vertex contacts, and collinear overlaps. Shared-junction contact is ignored only inside the source-resolution-aware junction tolerance; remote contact with an incident way is still blocking. Regression tests cover legacy precise apply, deferred node mutation, vertex self-touch, connected-way endpoint contact, collinear overlap, short-overlap grouping, predicted-signal isolation, interpolation-only motion, and unbounded sparse gaps.
