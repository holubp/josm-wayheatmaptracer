...
- Outlier strand handling must be conditional. Penalize narrow wandering side strands when a stronger center exists in the same cross-section, but do not discard narrow/low-intensity peaks globally because lightly used paths may only appear as individual strands.
...
- Multi-color detection should compare classifiers for consensus rather than simply listing independent per-color candidates. Consensus is primarily an agreement/ranking signal over stable per-color ridges. Only create fused consensus geometry when the agreeing ridges are close, smooth, and non-oscillating across the segment. Keep semantic dual-color classifiers useful when they agree with other modes; `gray` is dual-color too, not a plain grayscale brightness ramp.
- Broad heatmap conduits should be tracked through their longitudinal center. Do not let blue/gray shoulder or edge pixels make the result alternate between sides when a coherent corridor center is available.
- Fixed junction endpoints should preserve the selected way's incoming/outgoing approach direction. Do not let a brighter crossing or main-road heatmap blob introduce unsupported near-junction kinks.
...
- Managed sampling defaults to stable fixed-scale inference: primary ridge extraction must use smoothed source tiles at the configured inference zoom, with configured validation tiles used as a warning-level sanity check. Do not make unsmoothed raw high-resolution tiles the default ridge input again without fixture coverage proving stable behavior.
- Managed sampling keeps parameters meter-based, not raw-pixel-only, so behavior stays comparable across zoom levels.
- Managed candidate safety gates must distinguish hard rejection from low-confidence preview. Hard-reject structural failures and unusable selected-area tiles; keep sparse evidence, long no-signal gaps, weak lower-zoom validation, edge-of-band candidates, and large low-support displacement previewable with warnings and score penalties.
- Managed fixed-tile alignment should not re-enable iterative self-refinement unless there is regression coverage proving it cannot drift onto sparse or parallel traces.
...
- Last-slide debug export should prioritize the most recent slide attempt and include source tiles, redacted settings, sampled colors, original/preview/candidate geometry, candidate evidence, and per-slide logs. Never export cookies, signed headers, or full signed URLs.
