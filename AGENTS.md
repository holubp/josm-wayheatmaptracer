...
- Multi-color detection should compare classifiers for consensus rather than simply listing independent per-color candidates. True consensus means a fused geometry across agreeing color schemes, not a single color candidate with a consensus score bonus. Weight consensus by signal quality/SNR. Keep semantic dual-color classifiers useful when they agree with other modes; `gray` is dual-color too, not a plain grayscale brightness ramp.
